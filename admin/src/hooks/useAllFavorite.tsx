import type { FavoriteParams, Favorite } from "../types/favoriteType";
import { getAllFavoriteApi } from "../services/favoriteApi";
...
export const useAllFavorite = () => {
  ...
  const [filters, setFilters] = useState<FavoriteParams>({
  ...
  const updateURL = (newFilters: FavoriteParams) => {
  ...
  const updateFilter = (data: Partial<FavoriteParams>) => {
  ...
  useEffect(() => {
    const fetchAllUsers = async () => {
      ...
      setError(null);
      try {
        const data = await getAllFavoriteApi(filters);
